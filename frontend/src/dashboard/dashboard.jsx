...
<div className="grid md:grid-cols-3 lg:grid-cols-4 gap-6">
  {[
    ...
    <div
      key={i}
      className="bg-black/40 p-4 rounded-xl border border-white/5 hover:border-[#1FBCF9]/40 transition"
    >
      ...
      {isEditing ? (
        <input
          ...
          onChange={(e) =>
            setEditForm({ ...editForm, [stat.key]: e.target.value })
          }
        ...
        <p
          ...
          className={`font-semibold mt-1 
            ${
              stat.key === "email"
                ? "text-sm break-all"   // 👈 FIX for email
                : "text-xl break-words"
            }`}
        >
          {data[stat.key] || "N/A"}
